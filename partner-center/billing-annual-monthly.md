...
本文介绍合作伙伴中心中**每月计费**和**年度计费**之间的差异，包括福利和用例。 你可以选择按月或按年为某些云解决方案提供商（CSP）订阅付费。

## <a name="applicability"></a>适用范围

大多数基于授权的订阅都具有每月或每年计费选项的选项。 基于使用情况的订阅只有按月计费选项。

每年和每月计费都**按订阅**进行， **_而不_是按许可证**。
...
如果要调整计费日期以匹配单个客户的日期，则从年度计费切换到月度计费可能会很有用。
...
你将在下一个计费日期向你收费。 例如，如果你的帐单日期是月份的1号，而你在2019年10月29日购买了每年的帐单订阅，则将于2019年11月1日向你收费。 假设你不进行任何许可更改，你将在2020年11月1日再次向你收费。 如果你进行许可证更改，你将在下一个计费日期收到信用和 rebill。
...
对于具有相同产品/服务的**多个订阅**的客户，每个订阅可能有不同的计费频率。 有些产品/服务仅限每个客户一个订阅。 如果套餐不受限制，则每个客户可以拥有相同套餐的多份计费频率不同的订阅。 可以在套餐矩阵的 I 列查找所有套餐限制的详细信息。 你可以在合作伙伴中心的 "**查看产品和定价**" 部分中找到产品/服务。

### <a name="free-subscription-period"></a>免费订阅周期

具有年度计费频率的订阅不会获得免费的时间。 十二个月支付的期限从购买日期开始。 这不同于具有每月计费频率的订阅，该频率接收从购买日期到下一个计费日期的免费时间段。
...
对于年度计费，如果订阅在12个月支付期限的前30天内取消，你将在下一个计费日期收到100% 的信用额度。 如果在十二个月支付的期限30天后取消订阅，你将在下一个计费日期收到按比例信用额度。

### <a name="moving-subscriptions-between-partners"></a>在合作伙伴之间移动订阅

客户无法将订阅从一个伙伴移到另一个合作伙伴。 此限制适用于每月和每年计费的订阅。
...
如果产品/服务价格在计费期间降低，则不会更改你的计费金额。 此价格是在购买时为全计费期间设置的。 此约定适用于每月和每年计费。
...
- 续订订阅时，将显示对对帐文件的下一次更改。 此更改将在续订后的第一个计费日期显示。
...
将许可证添加到订阅时，对帐文件将包含贷记和按比例 rebill。 此约定适用于每月和每年计费的订阅。

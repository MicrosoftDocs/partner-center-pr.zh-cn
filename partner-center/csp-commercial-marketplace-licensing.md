...
### <a name="review-the-azure-marketplace-documentation"></a>查看 Azure Marketplace 文档

下面的文章包含您在继续操作之前应该了解的信息。 

- [创建 Dynamics 365 for Customer Engagement & PowerApps 套餐](https://docs.microsoft.com/azure/marketplace/dynamics-365-customer-engage-offer-setup)
- [在合作伙伴中心创建商业市场帐户](https://docs.microsoft.com/azure/marketplace/create-account)

### <a name="create-your-offer-id"></a>创建产品/服务 ID
...
- 只使用小写字母和数字。 产品/服务 ID 可以包括连字符和下划线，但不能包含空格。 例如，如果你的发布者 ID 是 "testpublisherid"，并且你输入了 "测试/服务-1"，则 "产品/服务" 网址为 https://appsource.microsoft.com/product/dynamics-365/testpublisherid.test-offer-1 。
- 选择 " **创建**" 后，不能更改此 ID。

### <a name="create-your-offer-alias"></a>创建产品/服务别名
...
## <a name="register-isv-connect-deal-in-dealreg"></a>注册 DealReg 中的 ISV Connect 交易

下一步是注册交易。 为此，请参阅 [注册交易](https://docs.microsoft.com/partner-center/register-deals)。
...
1. 登录到[合作伙伴中心仪表板](https://partner.microsoft.com/dashboard/)。
2. 在左侧导航菜单中，选择 " **商业 Marketplace/概述**"。
3. 筛选 **提交** 的交易，选择 " **正在进行** " 选项卡，然后选择所需的交易。
4. 在此交易的 "概述" 页上，选择 " **管理许可证**"。
5. 在 " **管理许可证** " 窗口中，从 " **客户详细信息** " 下拉列表中选择客户。 如果客户关系尚不存在，请选择 " **+ 邀请新客户同意**"。
6. 复制显示的链接。
7. 通过电子邮件将此链接发送到客户的帐单管理员或全局管理员，并让他们使用此链接访问 admin.microsoft.com，并接受并授权您正在建立的关系。
...
建立客户后，可以开始从产品/服务添加计划并将许可证分配给每个计划。

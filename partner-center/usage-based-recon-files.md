...
| Region | 使用情况适用的区域。 主要用于分配数据传输费率，因为费率因区域而异。 | *亚太*、*欧洲*、*拉丁美洲* *北美* |
...
| ConsumedQuantity | 报表期间使用的服务量（如小时或 GB）。 此外还包括来自以前报告期间任何未开票的使用量。 | *11* |
...
| 货币 | 货币类型。 每个计费单位仅使用一种货币。 请检查它是否与第一个发票匹配，然后再更新任何主要的计费平台。 | *EUR* |

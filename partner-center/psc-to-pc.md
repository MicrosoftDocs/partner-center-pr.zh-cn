...
## <a name="psc-deals-migration"></a>PSC 交易迁移

完成用户迁移后，使用 "共同销售机会" 页中的 "交易迁移向导" 将所有符合条件的开放交易从 PSC 引入 PC。 **"交易迁移" 链接将仅在合作伙伴中心的整个组织范围内可见。** "共同销售机会" 页的右上方会出现名为 **"PSC 交易迁移"** 的链接，这将打开 "交易迁移向导"。
...
合作伙伴中心的交易迁移只是一次单击体验。 只需在公司准备好迁移符合条件的交易后，单击 **"迁移交易"** 按钮即可。 **不能选择想要从 PSC 迁移的交易。如果你不想将任何交易迁移到合作伙伴中心，请在开始迁移之前将其移动到 PSC 中的已关闭状态。**

>[!Note]
> 启动迁移后， **最多需要24小时才能迁移交易**。

迁移完成后，横幅消息的状态将更改为 "已完成"，并提供指向迁移报表的链接。 下载报表，查看从 PSC 迁移到 PC 的交易的详细信息。

该报表包含以下详细信息。

1. **合作伙伴中心参与 ID** -参与中所有交易的合作伙伴中心中的唯一标识符。 有两个交易需要：一个用于合作伙伴，另一个用于在合作伙伴中心进行共同销售的合作。
2. **合作伙伴中心引用 ID** -属于合作伙伴的交易的合作伙伴中心中的唯一标识符。
3. **交易名称** -为 PSC 中的交易指定的标识符。

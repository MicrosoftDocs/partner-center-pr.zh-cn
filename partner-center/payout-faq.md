...
- 检查按国家/地区分类的税务责任。 在税务由 Microsoft 负责的国家/地区，Microsoft 将从发布者收入中收取和扣税。 对账单中显示的交易金额为税后金额。 请参阅[税项详细信息](tax-details-marketplace.md)。
- SaaS 和 IaaS 产品/服务有 10% 的折扣代理费用，而不是标准的 20%，保留 90% 的收入率。 这是 IP Co-Sell要求。 此促销在 2021 年 6 月 30 日之前有效。 

**进一步阅读**：[商业市场](/legal/marketplace/msft-publisher-agreement)发布者协议、[付款策略详细信息](payout-policy-details.md)、[付款阈值](payment-thresholds-methods-timeframes.md)、方法和时间范围、[获取付费](marketplace-get-paid.md)、[税务详细信息](tax-details-marketplace.md)[、付款对帐单](payout-statement.md)
...
将付款交易历史记录报告中出现的 AssetID、orderID 和行项 ID 与分析订单和使用情况报告一同使用。 使用此映射：
...
- 首先，使用 assetID 检查订单报告中 [的客户订单](https://partner.microsoft.com/dashboard/commercial-marketplace/analytics/order)。
- 在客户报告中检查客户订阅 [的客户渠道](https://partner.microsoft.com/dashboard/commercial-marketplace/analytics/customer)。
- 对于企业客户，发布者收益显示在采购订单日期后的 1-2 天内的对账单中。
- 对于非企业客户，发布者收益将在收到客户付款后的 1-2 天内显示在对账单中。

**进一步阅读**[：商业](payout-statement.md)市场 [分析中的付款对帐单、订单仪表板](/azure/marketplace/partner-center-portal/orders-dashboard)

## <a name="payments-and-adjustments"></a>付款和调整

#### <a name="why-is-my-payment-missing"></a>为何我的付款缺失？

- 确保"付款状态"和"税务配置文件状态"在 *概述* 页上 [显示为有效](https://partner.microsoft.com/dashboard/commercial-marketplace/overview)。
- 你可能未满足付款的最小阈值。 收入必须至少为 50 美元才能获得付款。


#### <a name="how-do-i-set-my-account-to-not-receive-payment"></a>如何实现帐户是否未收到付款？
可以在付款配置文件 中 [保存付款](https://partner.microsoft.com/dashboard/commercial-marketplace/overview)。 为此，请选中 **"保留"，Microsoft** 将保留付款，直到你释放暂停。
...
付款调整是用于容纳补偿调整（例如系统问题）的付款更正。 在付款语句中，ReasonCode 指定调整的原因。 这些不是直接协调单个事务。

**进一步阅读**： [商业 Marketplace 发布者协议](/legal/marketplace/msft-publisher-agreement)、 [支出详细信息](payout-policy-details.md)、 [税务详细信息](tax-details-marketplace.md)、 [支付阈值、方法和时间范围](payment-thresholds-methods-timeframes.md)
...
**进一步阅读**： [商业 Marketplace 发布者协议](/legal/marketplace/msft-publisher-agreement)， [详细信息策略详细信息](payout-policy-details.md)
...
使用 [合作伙伴付款 API](https://apidocs.microsoft.com/services/partnerpayouts)。

## <a name="commercial-marketplace-payout-policies"></a>商业市场付款策略

#### <a name="how-do-i-find-the-current-agency-fee-and-the-payout-rate"></a>如何查找当前的代理费和付款比率？

- 检查商业市场发布者协议。 标准代理费为 20%。 SaaS 联合销售符合条件的交易有 10% 的折扣费用。 检查是否有任何促销代理费的公告。
- 在付款对账单中，"收入率"指定给定交易的实际付款费率。
...
- 当你的收入处于未处理状态之后，你可以检查将处理你的收入以进行付款的月份的到期日期。 准备好付款后，收入状态将更改为"已处理"。  Microsoft 将在每个到期月份的 15 号之前发放付款。
- 对于使用信用卡支付的订单，Microsoft 将保留付款 30 天，直到收入到期。

 **进一步阅读**：[商业市场发布者协议](/legal/marketplace/msft-publisher-agreement)[、付款策略详细信息](payout-policy-details.md)、[税务详细信息](tax-details-marketplace.md)、[付款阈值](payment-thresholds-methods-timeframes.md)、方法和时间范围

...
- CRM 管理员，并在解决方案中自动执行流处理。 
    ...
    d. 使用 "引用管理员" 凭据角色为合作伙伴中心用户创建连接。

    e. 接下来，使用 "引用管理员" 的凭据为合作伙伴中心用户创建合作伙伴中心事件连接。
    
    f. 为 CRM 管理员用户 Common Data Service （当前环境）创建连接。
...
1. 若要自定义用于合作伙伴中心的创建或更新事件的字段以进行 CRM 引用同步，请执行以下操作： 
    ...
    a. 选择 "合作伙伴中心到 Salesforce CRM （有问必答预览版）"。
...
2. 若要为创建事件自定义 CRM 字段映射，请选择**新的共享机会，然后选择 "是"**。 选择 "**是"** ，然后**在 CRM 中展开 "创建新机会**"。 您可以使用字段映射指南来编辑此部分中的映射。
    ...
    d. 若要为更新事件自定义 CRM 字段映射，请单击 "（作用域）同步潜在客户或机会" 步骤。
...
•**审核**：与 Microsoft 合作伙伴中心引用同步的只读审核记录
...
|---------------------|:-------------------|:--------------|:----------------|
|与合作伙伴中心同步|与合作伙伴同步-中心-c|Checkbox （默认未选中）||
|产品|Products-c|文本（255）||
|引荐 | Referral_Identifier__c|文本（100）（外部 ID）|是|
|引用链接| Referral_Link__c_|URL （255）||
|审核| Audit__c|长文本区域（100000）（显示第4行）||
|Microsoft 如何帮助？|How_can_Microsoft_help__c|列表|
...
1. 在 CRM 中创建或更新引用并在伙伴中心同步时的引用同步：
    ...
    a. 在 CRM 的 "**机会**" 部分中具有可见性的用户登录到 Salesforce CRM 环境。

    b. 在 Salesforce CRM 环境中创建 "新机会" 时，请确保以下部分存在
    ...
    d. 将 "机会**与合作伙伴中心同步**" 选项设置为 **"是"** 后，请等待10分钟，登录到合作伙伴中心帐户。 你的引用将与 Salesforce CRM 同步。

    e. 当 "与合作伙伴中心同步" 选项设置为 "是" 时，如果您更新了 Salesforce CRM 中的机会，则这些更改将与合作伙伴中心帐户同步。

    f. 与合作伙伴中心成功同步的机会将用 Salesforce CRM 中的✔图标标识。
...
2. 引用同步在 Microsoft 合作伙伴中心创建或更新引用并在 Salesforce CRM 环境中同步时的同步： 
    ...
    d. 登录到 Salesforce CRM 环境。 

    e. 导航到 "**开放式机会**"。 Microsoft 合作伙伴中心中创建的引用现已在 Salesforce CRM 中同步。

...
如果需要请求奖励管理员权限，请联系 MPN 管理员或全局管理员。可以通过登录仪表板 来查找公司中具有这些角色 [合作伙伴中心角色](https://partner.microsoft.com/dashboard/)。 在右上角 **的"** 设置"图标中，选择" **用户管理"，** 然后筛选"全局管理员"。
...
>[!IMPORTANT]
>你必须是奖励管理员，才能创建或管理付款和税务配置文件合作伙伴中心。 奖励角色必须分配到每个奖励计划下的每个 MPN 位置。 有关如何在合作伙伴中心添加激励管理员的详细信息，请参阅 [创建用户帐户](create-user-accounts-and-set-permissions.md)。
...
1. 使用你的 Azure Active Directory (Azure AD) 帐户 (公司帐户) ，或使用适当的电子邮件地址（如果已分配）登录到 [合作伙伴中心仪表板](https://partner.microsoft.com/dashboard/) 。
   ...
   - 如果只能用域登录， @onmicrosoft.com 并且需要其他域，请与帐户管理员联系，将其他域添加到 Azure AD 帐户中。
...
可以设置默认银行配置文件并将其分配给 MPN 位置。 Microsoft 将使用这些默认配置文件进行该 MPN 位置的后续注册。 

1. 登录到仪表板[合作伙伴中心，](https://partner.microsoft.com/dashboard/)然后选择齿轮图标以打开"设置 **"**   菜单。 
...
>[!NOTE]
>同一个银行和货币配对可以应用于多个位置。 如果已将默认配置文件和货币组合分配给位置 MPN，则它将不再出现在将来的默认配置文件分配的 "位置" 下拉列表中。 如果删除了默认选择，则 MPN 将重新出现位置，以供将来的默认配置文件分配。 每个银行配置文件和货币组合将添加为唯一的可编辑行。

7. 添加所有必需的更改后，请选择 " **保存**"。  

## <a name="create-your-bank-profile"></a>创建银行配置文件

银行配置文件在公司级别创建。 这允许在一个公司内跨多个 MPN ID 和激励计划分配一个银行配置文件。 将银行配置文件应用于不同的国家/地区时，可能会出现例外，因为不同的银行和税务规则可能适用。

...
默认情况下，激励支付以各自实体的本地货币进行。 可以在配置文件安装过程中指定不同的货币。 将使用 Microsoft 每月设置的汇率来计算付款。 由于所选的货币，你将负责对值所做的任何更改。

### <a name="bank-and-tax-information-and-other-programs"></a>银行和税务信息以及其他程序

提供下面所述的信息，即使 Microsoft 已使用银行数据支付费用。 这有助于确保公司数据的隐私性和安全性，因为将您的配置文件复制到新工具可能会暴露敏感信息。 完成此过程也是确保数据完整且准确的好机会。

### <a name="using-different-profiles-for-different-microsoft-programs"></a>为不同的 Microsoft 程序使用不同的配置文件

在零售范围内，这五个零售激励计划中的每一项都可以发送到相同的银行帐户。 另外，还可以选择在零售办公室支付给不同银行帐户时，将零售 Xbox 付款传入一个银行帐户。

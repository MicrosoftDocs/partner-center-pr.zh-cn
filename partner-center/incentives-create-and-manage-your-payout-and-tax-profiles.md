...
## <a name="roles-currencies-and-multiple-microsoft-incentive-programs"></a>角色、货币和多个 Microsoft 激励计划
...
>[!IMPORTANT]
>您必须是激励管理员才能在合作伙伴中心创建或管理付款和税务配置文件。 奖励角色必须分配给每个激励计划下的每个 MPN 位置。 有关如何在合作伙伴中心添加激励管理员的详细信息，请参阅 [创建用户帐户](create-user-accounts-and-set-permissions.md)。
...
4. 选择 " **付款方式**"。
   ...
   - 如果已选择 " **贷方注释** " 作为付款方式，请完成验证。 这会确认引用的 SAP 编号属于你的组织。
    ...
    - 如果你的位置 MPN ID 由本地 Microsoft 子公司支付给特定激励计划，并允许 LRD (有限的风险分销商) 贷项通知单作为付款方式，则将使用 LRD 信用说明支付方法预填充你的付款配置文件。 在相应激励计划和位置 MPN ID 的 "LRD 贷方通知单支付方法" 行上，你将看到 "需要 **确认** " 或 " **验证** " 作为 "付款配置文件" 部分中的状态。
      ...
      目前，LRD 贷方通知单支付方法仅适用于 Microsoft 商业激励计划的澳大利亚、新西兰和加拿大合作伙伴。 如果你是注册了 MCI 计划的这三个国家/地区的直接帐单合作伙伴或间接提供商，但没有看到 LRD 贷方注释作为可用的付款方式，请确认你的租户 ID 与相关的合作伙伴 MPN 位置帐户关联。 有关此内容的详细信息，请参阅 [如何更新组织配置文件](update-your-partner-profile.md)。
...
可以设置默认的银行配置文件并将其分配到 MPN 位置。 Microsoft 将使用这些默认配置文件，以便以后注册该 MPN 位置。 

1. 登录到[合作伙伴中心仪表板](https://partner.microsoft.com/dashboard/)，并选择齿轮图标以打开 "设置" ****   菜单。 

2. 选择 " **帐户设置**"，展开 " **支出和税务** " 部分，然后选择 " **支出和税务配置文件**"。 

3. 选择 "**付款配置文件**" 部分下的 "**管理默认配置文件**"。 

4. 若要创建默认银行配置文件，请选择 " **添加默认银行配置文件**"。 

5. 从公司的可用银行配置文件列表中选择银行配置文件，选择要与此银行配置文件一起使用的货币，然后选择要应用此默认配置文件的 MPN 位置的列表。

6. 完成选择后，选择 " **完成** "。 完成所需的所有字段后，"完成" 按钮才会单击。 

>[!NOTE]
>同一银行和币种配对可以应用于多个位置。 如果已将默认配置文件和货币组合分配给位置 MPN，则它将不再出现在将来的默认配置文件分配的 "位置" 下拉列表中。 如果删除了默认选择，则 MPN 将重新出现位置，以供将来的默认配置文件分配。 每个银行配置文件和货币组合将添加为唯一的可编辑行。
...
1. 选择 " **帐户设置**"，展开 " **支出和税务** " 部分，然后选择 " **支出和税务配置文件**"。

1. 选择 " **税务配置文件**"。

1. 检查列 **到期日期** 并导航到过期的或即将过期的税务配置文件。

1. 选择“编辑”。

1. 在 "税务窗体" 部分中，通过提供新的详细信息来更新纳税形式。 

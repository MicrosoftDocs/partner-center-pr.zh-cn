...
5. 找到要取消的软件购买，然后选择 "**取消**"。 此时会显示一个对话框。

6. 从 "订单号" 下拉列表中，选择要取消的正确订单 ID 号，然后选择 "**提交**"。

7. 如果要为客户取消多个订单，则需要对每个唯一的订单 ID 号再次执行步骤4到步骤6。 （您可以从客户的**订单历史记录**页中了解有关订单或订单 ID 号的详细信息。）

8. 当你尝试取消订单时，合作伙伴中心还可能会向你显示其他信息（显示在 "订单号" 下拉列表下）。 此信息可能包括：
